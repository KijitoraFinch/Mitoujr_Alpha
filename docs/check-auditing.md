...
re-reads interpreted artifacts through the retained-handle boundary, and emits
...
- `whole-artifact` for a safely readable target;
...
selector. Digest expectations apply to the entire target artifact identity,
...
produces `unreferenced-ref`; a missing artifact or selector with no match
produces `unresolved-ref`; malformed or ambiguous selector execution produces
`invalid-selector`; and a resolved artifact whose identity violates a digest
